...
async def reconcile_live_positions(
    ...
    *,
    ...
    for row in candidates:
        ...
        # Fallback: wallet inventory is flat but no sell trade found for this
        # specific token_id. This happens when Polymarket records the sell under
        # a different token_id variant (token versioning / condition mapping) or
        # when the position was closed via merge/redeem.  If the wallet is
        # verified flat and the position is old enough not to be a race, close it.
        if (
            token_id
            and entry_fill_size > 0.0
            ...
            if _has_been_evaluated or _position_age_minutes >= 5.0:
                close_price = (
                    wallet_mark_price
                    if wallet_mark_price is not None and wallet_mark_price > 0.0
                ...
                if close_price is not None and close_price > 0.0:
                    ...
                    if not dry_run:
                        ...
                        payload["position_close"] = {
                            "close_price": close_price,
                            "price_source": "wallet_flat_no_trade",
                            "close_trigger": close_trigger,
                            "realized_pnl": pnl,
                            "filled_size": close_qty,
                            "closed_at": _iso_utc(now),
