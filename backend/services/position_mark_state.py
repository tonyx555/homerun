...
class PositionMarkState:
    ...
    def register_position(
        ...
        with self._lock:
            existing = self._positions.get(order_id)
            if existing is not None:
                # Update mutable fields but preserve accumulated mark data
                existing.market_id = market_id
                existing.token_id = token_id
                existing.direction = direction.lower()
    ...
    def on_price_update(
        ...
        with self._lock:
            ...
            if not order_ids:
                ...
                if self._miss_count <= 5:
                    logger.debug(
                        "on_price_update miss: token=%s not in %d tracked tokens",
                        token_id[:12], len(self._token_to_orders),
                    )
                return
            self._sequence += 1
            ...
            for oid in order_ids:
                ...
                if mark is None:
                    continue
                # mid is already the price of the specific token subscribed
                # (each direction has its own token_id), no inversion needed
                mark_price = mid
        ...
        if changed:
            ...
            if self._update_count <= 5:
                logger.info(
                    "on_price_update HIT: token=%s mid=%.4f changed=%d positions (update #%d, callback=%s)",
                    token_id[:12], mid, len(changed), self._update_count,
                    "set" if self._on_marks_changed else "NOT SET",
                )
            if self._on_marks_changed:
                try:
                    self._on_marks_changed(changed)
                except Exception as exc:
                    logger.warning("on_marks_changed callback failed", exc_info=exc)
