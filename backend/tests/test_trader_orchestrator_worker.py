...
@pytest.mark.asyncio
async def test_run_trader_once_skips_tail_static_failures_before_strict_ws_defer(monkeypatch):
    signal = _base_signal()
    signal.source = "scanner"
    signal.signal_type = "scanner_opportunity"
    signal.strategy_type = "tail_end_carry"
    signal.direction = "buy_no"
    signal.entry_price = 0.945
    signal.market_id = "scanner-market-1"
    signal.market_question = "Spread: Aalesunds FK (-2.5)"
    signal.created_at = datetime.now(timezone.utc)
    signal.updated_at = datetime.now(timezone.utc)
    signal.liquidity = 1714.5239
    signal.payload_json = {
        "signal_emitted_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "ingested_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
        "strategy": "tail_end_carry",
        "markets": [
            {
                "id": "scanner-market-1",
                "question": "Spread: Aalesunds FK (-2.5)",
                "condition_id": "condition-1",
                "clob_token_ids": ["token-yes", "token-no"],
                "current_yes_price": 0.055,
                "current_no_price": 0.945,
                "liquidity": 1714.5239,
                "volume": 327.9017,
    ...
    signal.strategy_context_json = {
        "market_category": "sports",
        "is_spread_market": True,
        "liquidity_ok": False,
        "sports_window_ok": False,
        "live_game_detected": False,
        "panic_guard_ok": True,
        "book_spread": 0.0,
        "book_spread_ok": True,
        "max_settlement_upside_pct": 5.82,
        "upside_ok": True,
        "sports_min_probability": 0.9,
    }
    ...
    assert processed_signals == 1
    assert decisions[0]["decision"] == "skipped"
    assert "Spread markets blocked" in decisions[0]["reason"]
    defer_signal_mock.assert_not_awaited()
    record_consumption_mock.assert_awaited_once()
    cursor_mock.assert_awaited_once()
    assert heartbeat_calls == [] or heartbeat_calls[-1]["message"] != "Idle cycle: pending signals deferred awaiting live market context."


def test_tail_end_carry_checks_use_live_trader_params_over_scanner_flags():
    signal = SimpleNamespace(
        source="scanner",
        strategy_type="tail_end_carry",
        entry_price=0.945,
        ...
        payload_json={
            "strategy": "tail_end_carry",
            "resolution_date": (datetime.now(timezone.utc) + timedelta(hours=4, minutes=48)).isoformat(),
            "markets": [
                {
                    "id": "scanner-market-1",
                    "question": "Exact Score: Real Madrid CF 1 - 0 FC Bayern Munchen?",
                    "liquidity": 3363.5494,
                }
            ],
            "positions_to_take": [
                {
                    ...
                    "token_id": "token-no",
                    "_tail_end": {
                        "days_to_resolution": 0.20,
                        "spread": 0.0,
                        "probability": 0.945,
                        "max_settlement_upside_pct": 5.82,
                        "market_category": "sports",
                        "blocked_keyword": None,
                        "is_spread_market": False,
                        "liquidity_ok": False,
                        "sports_window_ok": False,
                        "live_game_detected": False,
                        "panic_guard_ok": True,
        ...
        strategy_context_json={
            "market_category": "sports",
            "execution_min_liquidity": 3500.0,
            "liquidity_ok": False,
            "sports_min_probability": 0.9,
            "sports_window_ok": False,
            "live_game_detected": False,
    ...
    payload = dict(signal.payload_json)
    params = {
    ...
    assert [check.key for check in checks if not check.passed] == []
